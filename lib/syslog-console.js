            ...
            for (var i in mMessage) {
                ...
                try {
                    if (oMessage instanceof Error) {
                        ...
                        delete mMessage[i];
                        this._send({action: "Error Details", error: oError}, this.oClient.LOG_ERR);
                    } else {
                        this._findErrors(oMessage);
                    }
                } catch (e) {
                    console.error('SYSLOG_ERROR', 'Had Some Trouble finding Errors');
                    console.error('SYSLOG_ERROR', e);
                    console.error('SYSLOG_ERROR_MESSAGE', oMessage);
                }
            }
        }
    };
